...
**Index entries must use the post's `title` as the link text and `subtitle` as the description.** Do not write custom descriptions — pull them from the post frontmatter. Build entries include a demo link if available, or a repo link if not. Standalone posts don't need either.
